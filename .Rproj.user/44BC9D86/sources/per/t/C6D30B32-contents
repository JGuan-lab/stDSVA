...
CDSC <- function(data_bulk,data_ref, k, lambda1, lambda2,lambdaC,error= 10^-8,seedd=44,Ss,Sg,all_number = 3000,ReturnL = TRUE,MASK = NULL,l=0.001,P.Pseudo=NULL){
  ...
  n <- gn[2]
    ...
    colnames(p) <- colnames(data_bulk)
    
    if(!is.null(P.Pseudo)){
      p[,colnames(P.Pseudo)] <- P.Pseudo
    }
  ...
  while(jump < all_number){
    ...
    if(is.null(MASK)){
      ...
      LAMBDA <- matrix(l,nrow = ncol(data_ref), ncol = ncol(data_bulk))
      LAMBDA <- LAMBDA * (1 - MASK)
      c <- c*((data_bulk%*%t(p)+ lambdaC*data_ref +2*lambda2*Sg%*%c))/(c%*%p%*%t(p)+ lambdaC*c+ 2*lambda2*c%*%t(c)%*%c)
      p <- p*(t(c)%*%data_bulk+2*lambda1*p%*%Ss)/(t(c)%*%c%*%p+2*lambda1*p%*%t(p)%*%p + LAMBDA * p * LAMBDA)
    }
    jump = jump+1
...
CDSC_without_pseudo <- function(data_bulk,data_ref, k, lambda1, lambda2,lambdaC,error= 10^-8,seedd=44,Ss,Sg,all_number = 3000,ReturnL = TRUE,MASK = NULL,l=0.001){
  data_bulk <- as.matrix(data_bulk)
  if(storage.mode(data_bulk) != "double"){
    storage.mode(data_bulk) <- "double"
  }
